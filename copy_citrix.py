...
def _run_typing(args, status_callback=None, progress_callback=None):
    clip_text = pyperclip.paste()
    if not clip_text:
        raise RuntimeError("Clipboard is empty.")

    total = _count_typeable_chars(str(clip_text))
    if progress_callback is not None:
    ...
    if status_callback is not None:
        status_callback("Simulating keypresses...")
    _type_clipboard_text(str(clip_text), args.key_delay, progress_callback=progress_callback)
    if status_callback is not None:
...
def _run_cli(args):
    ...
    _run_typing(args, status_callback=_status, progress_callback=_progress)
    print()


def main():
    parser = argparse.ArgumentParser(
        description="Paste clipboard text by simulating key presses, with optional GUI progress."
    )
    parser.add_argument(
        "--key-delay",
        type=float,
